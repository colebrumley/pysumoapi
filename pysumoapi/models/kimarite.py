...
class KimariteRecord(BaseModel):
    ...
    last_usage: str = Field(..., alias="lastUsage", description="Last basho and day this kimarite was used (format: YYYYMM-DD)")
...
class KimariteResponse(BaseModel):
    ...
    sort_field: Optional[str] = Field(None, alias="sortField", description="Field to sort by")
    sort_order: Optional[Literal["asc", "desc"]] = Field("asc", alias="sortOrder", description="Sort order")
...
class KimariteMatchesResponse(BaseModel):
    ...
    records: List[KimariteMatch] = Field(..., description="List of matches where the kimarite was used") 